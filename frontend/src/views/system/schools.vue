        ...
        <el-table-column type="expand">
          <template #default="{ row }">
            <div class="expand-content">
              ...
              <div v-loading="row.departmentLoading" class="department-table">
                ...
                <div v-if="!row.departments || row.departments.length === 0" class="empty-departments">
...
const handleExpandChange = async (row, expandedRows) => {
  console.log('表格展开事件触发:', row.id, '展开状态:', expandedRows.includes(row))

  if (expandedRows.includes(row)) {
    // 行展开时加载院系数据
    await loadDepartments(row)
  }
}

// 加载院系数据
const loadDepartments = async (row) => {
  ...
  // 设置加载状态
  row.departmentLoading = true

  try {
    console.log('正在加载院系信息:', row.id)
    const response = await getDepartmentsBySchool(row.id)

    // 更新行数据
    row.departments = response.data || []

    // 强制更新表格
    tableData.value = [...tableData.value]

    console.log('院系数据加载完成:', response.data?.length || 0, '个院系')
  } catch (error) {
    console.error('加载院系数据失败:', error)
    ElMessage.error('加载院系数据失败')
    row.departments = []
  } finally {
    row.departmentLoading = false
  }
}
...
const loadData = async () => {
  ...
  try {
    ...
    tableData.value = content.map(item => ({
      ...item,
      departments: null, // 初始化为null，表示未加载
